...
def from_coffea(inpath, outfile):
    ...
    for year in [2017,2018]:
        # Get variations as a function of mjj and gen level V-pt
        distributions = ['mjj', 'gen_v_pt']
        for dist in distributions:
            # QCD V
            h_z = acc[dist][re.compile(f'ZJetsToNuNu.*HT.*{year}')].integrate('region', 'sr_vbf').integrate('dataset')
            f[f'z_qcd_{dist}_nominal_{year}'] = export1d(h_z)

            h_w = acc[dist][re.compile(f'WJetsToLNu.*HT.*{year}')].integrate('region', 'sr_vbf').integrate('dataset')
            f[f'w_qcd_{dist}_nominal_{year}'] = export1d(h_w)

            h_ph = acc[dist][re.compile(f'GJets_DR-0p4.*HT.*{year}')].integrate('region', 'cr_g_vbf').integrate('dataset')
            f[f'gjets_qcd_{dist}_nominal_{year}'] = export1d(h_ph)

            # Scale + PDF variations for QCD Z 
            h_z_unc = acc[f'{dist}_unc'][re.compile(f'ZJ.*HT.*{year}')].integrate('region', 'sr_vbf').integrate('dataset')
            for unc in map(str, h_z_unc.axis('uncertainty').identifiers()):
                ...
                f[f'z_qcd_{dist}_{unc}_{year}'] = export1d(h)

            # EWK variations for QCD Z
            # Get EWK down variation first
            h_z_unc_ewk = acc[f'{dist}_noewk'][re.compile(f'ZJetsToNuNu.*HT.*{year}')].integrate('region', 'sr_vbf').integrate('dataset')
            f[f'z_qcd_{dist}_unc_w_ewkcorr_overz_common_down_{year}'] = export1d(h_z_unc_ewk)
            ...
            # Get EWK down variation first
            h_w_unc_ewk = acc[f'{dist}_noewk'][re.compile(f'WJetsToLNu.*HT.*{year}')].integrate('region', 'sr_vbf').integrate('dataset')
            f[f'w_qcd_{dist}_unc_w_ewkcorr_overz_common_down_{year}'] = export1d(h_w_unc_ewk)
            ...
            for unc in map(str, h_ph_unc.axis('uncertainty').identifiers()):
                ...
                f[f'gjets_qcd_{dist}_{unc}_{year}'] = export1d(h)

            # EWK variations for QCD photons
            # Get EWK down variation first
            h_ph_unc_ewk = acc[f'{dist}_noewk'][re.compile(f'GJets_DR-0p4.*HT.*{year}')].integrate('region', 'cr_g_vbf').integrate('dataset')
            f[f'gjets_qcd_{dist}_unc_w_ewkcorr_overz_common_down_{year}'] = export1d(h_ph_unc_ewk)
            ...
            f[f'z_ewk_{dist}_nominal_{year}'] = export1d(h_z)
    
            h_w = acc[dist][re.compile(f'.*EWKW.*{year}')].integrate('region', 'sr_vbf').integrate('dataset')
            f[f'w_ewk_{dist}_nominal_{year}'] = export1d(h_w)
    
            h_ph = acc[dist][re.compile(f'GJets_SM_5f_EWK.*{year}')].integrate('region', 'cr_g_vbf').integrate('dataset')
            f[f'gjets_ewk_{dist}_nominal_{year}'] = export1d(h_ph)
            print(h_ph.values())

            # Scale + PDF variations for EWK Z
            h_z_unc = acc[f'{dist}_unc'][re.compile(f'.*EWKZ.*{year}')].integrate('region', 'sr_vbf').integrate('dataset')
            for unc in map(str, h_z_unc.axis('uncertainty').identifiers()):
                if 'goverz' in unc or 'ewkcorr' in unc:
                    continue
                h = h_z_unc.integrate(h_z_unc.axis('uncertainty'), unc)
                f[f'z_ewk_{dist}_{unc}_{year}'] = export1d(h)

            # Scale + PDF variations for EWK photons
            h_ph_unc = acc[f'{dist}_unc'][re.compile(f'GJets_SM.*{year}')].integrate('region', 'cr_g_vbf').integrate('dataset')
            for unc in map(str, h_ph_unc.axis('uncertainty').identifiers()):
                if 'zoverw' in unc or 'ewkcorr' in unc:
                    continue
                h = h_ph_unc.integrate(h_ph_unc.axis('uncertainty'), unc)
                f[f'gjets_ewk_{dist}_{unc}_{year}'] = export1d(h)
...
def make_ratios(infile):
    ...
    for year in [2017,2018]:
        for dist in ['mjj','gen_v_pt']:
            for vartype in ['up','down']:
                varied_z_name = f'z_qcd_{dist}_unc_w_ewkcorr_overz_common_{vartype}_{year}'
                varied_w = f.Get(f'w_qcd_{dist}_unc_w_ewkcorr_overz_common_{vartype}_{year}')
                varied_ratio = f.Get(varied_z_name).Clone(f'ratio_{varied_z_name}')
                varied_ratio.Divide(varied_w)
                varied_ratio.SetDirectory(of)
                varied_ratio.Write()  

            nominal_z_name = f'z_qcd_{dist}_nominal_{year}'
            nominal_w = f.Get(f'w_qcd_{dist}_nominal_{year}')
    ...
    for year in [2017,2018]:
        for dist in ['mjj','gen_v_pt']:
            for vartype in ['up','down']:
                varied_g_name = f'gjets_qcd_{dist}_unc_w_ewkcorr_overz_common_{vartype}_{year}'
                varied_z = f.Get(f'z_qcd_{dist}_unc_w_ewkcorr_overz_common_{vartype}_{year}')
                varied_ratio = f.Get(varied_g_name).Clone(f'ratio_{varied_g_name}')
                varied_ratio.Divide(varied_z)
                varied_ratio.SetDirectory(of)
                varied_ratio.Write()

            nominal_g_name = f'gjets_qcd_{dist}_nominal_{year}'
            nominal_z = f.Get(f'z_qcd_{dist}_nominal_{year}')
...
def make_uncertainties(infile):
    ...
    for source in ['ewk','qcd']:
        for year in [2017,2018]:
            for dist in ['mjj', 'gen_v_pt']:
                nominal = f.Get(f'ratio_z_{source}_{dist}_nominal_{year}')
                for name in map(lambda x:x.GetName(), f.GetListOfKeys()):
                    m = bool(re.match(f'.*z_{source}_{dist}_unc_(.*)_{year}', name)) and 'ewkcorr' not in name
                    if not m:
                        continue
                    ratio = f.Get(name)
    ...
    for year in [2017,2018]:
        for dist in ['mjj', 'gen_v_pt']:
            nominal = f.Get(f'ratio_z_qcd_{dist}_nominal_{year}')
            for vartype in ['up', 'down']:
                varied_name = f'ratio_z_qcd_{dist}_unc_w_ewkcorr_overz_common_{vartype}_{year}'
    ...
    for year in [2017,2018]:
        for dist in ['mjj', 'gen_v_pt']:
            for vartype in ['up', 'down']:
                qcd_unc_name = f'uncertainty_ratio_z_qcd_{dist}_unc_w_ewkcorr_overz_common_{vartype}_{year}'
    ...
    for source in ['ewk','qcd']:
        for year in [2017,2018]:
            for dist in ['mjj', 'gen_v_pt']:
                nominal = f.Get(f'ratio_gjets_{source}_{dist}_nominal_{year}')
                for name in map(lambda x:x.GetName(), f.GetListOfKeys()):
                    m = bool(re.match(f'.*gjets_{source}_{dist}_unc_(.*)_{year}', name)) and 'ewkcorr' not in name
                    if not m:
                        continue
                    ratio = f.Get(name)
                    variation = ratio.Clone(f'uncertainty_{name}')
    ...
    for year in [2017,2018]:
        for dist in ['mjj', 'gen_v_pt']:
            nominal = f.Get(f'ratio_gjets_qcd_{dist}_nominal_{year}')
            for vartype in ['up', 'down']:
                varied_name = f'ratio_gjets_qcd_{dist}_unc_w_ewkcorr_overz_common_{vartype}_{year}'
                varied = f.Get(varied_name)
                # Variation: (varied Z / W) / (nominal Z / W)
                variation = varied.Clone(f'uncertainty_{varied_name}')
                variation.Divide(nominal)
    ...
    for year in [2017,2018]:
        for dist in ['mjj', 'gen_v_pt']:
            for vartype in ['up', 'down']:
                qcd_unc_name = f'uncertainty_ratio_gjets_qcd_{dist}_unc_w_ewkcorr_overz_common_{vartype}_{year}'
                qcd_unc = of.Get(qcd_unc_name)
                ewk_unc = qcd_unc.Clone(f'{qcd_unc_name.replace("qcd", "ewk")}')
