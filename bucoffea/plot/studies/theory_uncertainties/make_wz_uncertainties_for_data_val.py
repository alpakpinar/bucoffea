            ...
            for name in map(lambda x:x.GetName(), f.GetListOfKeys()):
                ...
                if name.startswith(f'zmumu_{source}') and 'zoverw' in name:
                    ...
                    if 'nominal' in name:
                        ...
                        unc_tag = re.findall('unc_.*', name)[0]
                    # Ratio: Z(ee) / W(enu)
                    ratio_e = f.Get(name).Clone(f'zee_over_wenu_{unc_tag}')
...
def make_uncertainties(infile, run_z_w_only=False, run_qcd_only=False, variable='mjj'):
    ...
    for source in sources:
        for year in [2017,2018]:
            ...
            zee_over_wenu_nominal = f.Get(f'zee_over_wenu_nominal_{year}')
            zmumu_over_wmunu_nominal = f.Get(f'zmumu_over_wmunu_nominal_{year}')
            zmumu_over_gjets_nominal = f.Get(f'zmumu_over_gjets_nominal_{year}')
            zee_over_gjets_nominal = f.Get(f'zee_over_gjets_nominal_{year}')
            wmunu_over_gjets_nominal = f.Get(f'wmunu_over_gjets_nominal_{year}')
            wenu_over_gjets_nominal = f.Get(f'wenu_over_gjets_nominal_{year}')
            for name in map(lambda x:x.GetName(), f.GetListOfKeys()):
                # Uncertainties for Z(mumu) / W(munu)
                if re.match(f'.*zmumu_over_wmunu_unc_(.*)_{year}', name):
                ...
                elif re.match(f'.*zee_over_wenu_unc_(.*)_{year}', name):
                ...
                elif re.match(f'.*zmumu_over_gjets_unc_(.*)_{year}', name):
                ...
                elif re.match(f'.*zee_over_gjets_unc_(.*)_{year}', name):
                ...
                elif re.match(f'.*wmunu_over_gjets_unc_(.*)_{year}', name):
                ...
                elif re.match(f'.*wenu_over_gjets_unc_(.*)_{year}', name):
    ...
    for source in sources:
        for year in [2017,2018]:
            ...
            for unc_source in unc_sources:
                print(unc_source)
                unc_zmumu_over_wmunu = f.Get(f'uncertainty_zmumu_over_wmunu_{unc_source}')
                unc_zee_over_wenu = f.Get(f'uncertainty_zee_over_wenu_{unc_source}')
                unc_zmumu_over_gjets = f.Get(f'uncertainty_zmumu_over_gjets_{unc_source}')
                unc_zee_over_gjets = f.Get(f'uncertainty_zee_over_gjets_{unc_source}')
                unc_wmunu_over_gjets = f.Get(f'uncertainty_wmunu_over_gjets_{unc_source}')
                unc_wenu_over_gjets = f.Get(f'uncertainty_wenu_over_gjets_{unc_source}')

                # Combine in quadrature: Z / W
                combined_unc_zll_over_wlnu = f.Get(f'uncertainty_zmumu_over_wmunu_{unc_source}').Clone(f'uncertainty_zll_over_wlnu_{unc_source}')
                ...
                combined_unc_zll_over_gjets = f.Get(f'uncertainty_zmumu_over_gjets_{unc_source}').Clone(f'uncertainty_zll_over_gjets_{unc_source}')
                ...
                # combined_unc_wlnu_over_gjets = f.Get(f'uncertainty_wmunu_over_gjets_{unc_source}').Clone()
                # for idx in range(1, unc_zmumu_over_gjets.GetNbinsX()+1):
                    # wmunu_over_gjets_unc = unc_wmunu_over_gjets.GetBinContent(idx)
                    # wenu_over_gjets_unc = unc_wenu_over_gjets.GetBinContent(idx)
                    # combined_unc_wlnu_over_gjets.SetBinContent(idx, np.sqrt( (1-wmunu_over_gjets_unc)**2 + (1-wenu_over_gjets_unc)**2))
