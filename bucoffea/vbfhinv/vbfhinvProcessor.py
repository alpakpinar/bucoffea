...
class vbfhinvProcessor(processor.ProcessorABC):
    ...
    def process(self, df):
        ...
        df['has_lhe_v_pt'] = df['is_lo_w'] | df['is_lo_z'] | df['is_nlo_z'] | df['is_nlo_w'] | df['is_lo_g'] | df['is_lo_z_ewk'] | df['is_lo_w_ewk']
        df['is_data'] = is_data(dataset)

        gen_v_pt = None
        n_gen_dilepton = np.zeros(df.size)
        if df['is_lo_w'] or df['is_lo_z'] or df['is_nlo_z'] or df['is_nlo_w'] or df['is_lo_w_ewk'] or df['is_lo_z_ewk']:
