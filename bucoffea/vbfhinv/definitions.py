...
def vbfhinv_accumulator(cfg):
    ...
    jet_pt_ax_coarse = Bin("jetpt", r"$p_{T}$ (GeV)", 25, 0, 500)
    ...
    items["ak4_pt0_mjj"] = Hist("Counts", dataset_ax, region_ax, jet_pt_ax_coarse, mjj_ax)
    items["ak4_pt1_eta1"] = Hist("Counts", dataset_ax, region_ax, jet_pt_ax, jet_eta_ax_coarse)
    items["ak4_pt1_mjj"] = Hist("Counts", dataset_ax, region_ax, jet_pt_ax_coarse, mjj_ax)
...
def vbfhinv_regions(cfg):
    ...
    # SR regions with the leading jet in endcap (positive half or negative half)
    regions['sr_vbf_ak40_pos_endcap'] = copy.deepcopy(regions['sr_vbf_no_veto_all'])
    regions['sr_vbf_ak40_pos_endcap'].append('ak40_pos_endcap')
    
    regions['sr_vbf_ak40_neg_endcap'] = copy.deepcopy(regions['sr_vbf_no_veto_all'])
    regions['sr_vbf_ak40_neg_endcap'].append('ak40_neg_endcap')
